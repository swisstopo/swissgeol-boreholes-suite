...
public class SyncBoreholesTaskTest
{
    ...
    public async Task SyncBoreholes()
    {
        ...
        await syncContext.Source.SetBorholePublicationStatusAsync(1_000_001, 1, Role.Editor, cancellationToken);
        await syncContext.Source.SetBorholePublicationStatusAsync(1_000_202, 1, Role.Controller, cancellationToken);
        await syncContext.Source.SetBorholePublicationStatusAsync(1_001_404, 1, Role.Validator, cancellationToken);

        await syncContext.Source.SetBorholePublicationStatusAsync(1_000_022, 1, Role.Publisher, cancellationToken);
        await syncContext.Source.SetBorholePublicationStatusAsync(1_000_099, 1, Role.Publisher, cancellationToken);
        await syncContext.Source.SetBorholePublicationStatusAsync(1_000_101, 1, Role.Publisher, cancellationToken);

        // Remove all boreholes from the target database
        syncContext.Target.RemoveRange(syncContext.Target.Boreholes);
    ...
    public async Task SyncBoreholesForUndefinedDefaultWorkgroup()
    {
        ...
        var publishedBorehole = new Borehole().SetBorholePublicationStatus(Role.Publisher);
