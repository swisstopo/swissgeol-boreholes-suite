...
internal static class TestSyncContextExtensions
{
    ...
    internal static async Task<Borehole> SetBorholePublicationStatusAsync(this BdmsContext context, int boreholeId, int userId, Role status, CancellationToken cancellationToken)
    {
        ...
        borehole.SetBorholePublicationStatus(status);
    ...
    /// two serlialized <see cref="Borehole"/> objects.
    /// </summary>
    ...
    /// fields (e.g. ids, users, workflows/workgroup) are omited in order to be able to compare the raw
    ...
    /// are exptected to be also available in <see cref="Completion"/>s. Our test data seeded with
    ...
    /// <summary>
    /// Purges all <see cref="Casing"/> refernces in the given <paramref name="borehole"/>.
    /// </summary>
    ...
    /// <summary>
    /// Purges all <see cref="Casing"/> referencess for all objects in the given list of <paramref name="casingReferences"/>.
    /// </summary>/param>
    ...
    /// <summary>
    /// Fixes faulty <see cref="Casing"/> refernces in the given <paramref name="borehole"/>.
    /// </summary>
    private static void FixFaultyCasingReferences(this Borehole borehole)
    {
        ...
        var allreadyExistingCompletionCasingIds = borehole.Completions
        ...
        foreach (var referencedCasing in referencedCasings)
        {
            ...
            if (!allreadyExistingCompletionCasingIds.Contains(referencedCasing.Id))
            {
    ...
    /// <summary>
    /// Fixes faulty <see cref="Casing"/> refernces in the given list of <paramref name="casingReferences"/>.
    /// </summary>
