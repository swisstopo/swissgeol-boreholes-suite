...
public class LogFileController : ControllerBase
{
    ...
    /// <summary>
    /// Exports one or multiple log files.
    /// </summary>
    /// <param name="logFileIds">The IDs of the log files to export.</param>
    /// <returns>The file content or a zip file with multiple files.</returns>
    [HttpGet("export")]
    [Authorize(Policy = PolicyNames.Viewer)]
    public async Task<ActionResult> ExportAsync([FromQuery][MaxLength(100)] IReadOnlyList<int> logFileIds)
    {
        ...
        if (!await boreholePermissionService.CanViewBoreholeAsync(HttpContext.GetUserSubjectId(), boreholeId).ConfigureAwait(false))
        {
            return Unauthorized();
        }
        ...
        if (logFiles.Count == 1)
        {
            ...
            return File(fileBytes, logFile.FileType, logFile.Name);
        }
        ...
        using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
        {
            foreach (var logFile in logFiles)
            {
                ...
                var zipEntry = archive.CreateEntry($"{logFile.NameUuid}_{logFile.Name}", CompressionLevel.Fastest);
                using var zipEntryStream = zipEntry.Open();
                await zipEntryStream.WriteAsync(fileBytes.AsMemory(0, fileBytes.Length)).ConfigureAwait(false);
            }
        }

        return File(memoryStream.ToArray(), "application/zip", "logfiles.zip");
    }
    ...
    public async Task<ActionResult> UpdateAsync([FromBody] Collection<LogFileUpdate> logFileUpdates)
    {
        ...
        foreach (var logFile in logFiles)
        {
            ...
            if (updateData != null)
            {
                ...
                if (updateData.Pass.HasValue)
                {
                    logFile.Pass = updateData.Pass.Value;
                }

                if (updateData.DeliveryDate.HasValue)
                {
                    logFile.DeliveryDate = updateData.DeliveryDate.Value;
                }

                if (updateData.PassTypeId.HasValue)
                {
                    logFile.PassTypeId = updateData.PassTypeId.Value;
                }

                if (updateData.DataPackageId.HasValue)
                {
                    logFile.DataPackageId = updateData.DataPackageId.Value;
                }

                if (updateData.DepthTypeId.HasValue)
                {
                    logFile.DepthTypeId = updateData.DepthTypeId.Value;
                }
            }
        }
