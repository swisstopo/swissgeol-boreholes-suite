...
import { goToRouteAndAcceptTerms, handlePrompt } from "../helpers/testHelpers";
...
describe("User administration settings tests", () => {
  ...
  it("shows workgroup detail inactivates and activates workgroup", () => {
    ...
    cy.dataCy("settings-header").should("contain", "World");
    ...
    cy.dataCy("workgroup-detail").should("have.css", "opacity", "0.5");
    cy.dataCy("activate-button").click();
    cy.wait("@update-workgroup");
    cy.dataCy("workgroup-detail").should("have.css", "opacity", "1");
    ...
    clickOnRowWithText("World");
    cy.dataCy("deleteworkgroup-button").click();
    handlePrompt(activeWorkgroupDeletePrompt, "cancel");
  ...
  it("can add and edit a new workgroup.", () => {
    ...
    cy.wait("@update-workgroup");
    cy.dataCy("settings-header").contains("Coconut Updated");
  ...
  it("adds and deletes users from workgroup.", () => {
    ...
    cy.dataCy("adduser-button").click();
  ...
  it("displays error alert when updating workgroup fails.", () => {
    ...
    cy.dataCy("inactivate-button").should("have.class", "Mui-selected");
    cy.dataCy("activate-button").should("not.have.class", "Mui-selected");
  });
