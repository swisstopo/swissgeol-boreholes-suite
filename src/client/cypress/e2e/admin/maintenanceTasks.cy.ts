import { goToRouteAndAcceptTerms, loginAsAdmin, loginAsEditor, selectLanguage } from "../helpers/testHelpers";

const TASK_TYPES = ["LocationMigration", "CoordinateMigration", "UserMerge"];

const makeTaskState = (type: string, overrides: Record<string, unknown> = {}) => ({
  type,
  status: "Idle",
  affectedCount: null,
  message: null,
...
const makeStatusResponse = (overridesByType: Record<string, Record<string, unknown>> = {}) =>
  TASK_TYPES.map(type => makeTaskState(type, overridesByType[type]));

const makeLogResponse = (logEntries: Record<string, unknown>[] = [], totalCount: number | null = null) => ({
...
const interceptStatus = (body: unknown, alias: string) => {
  cy.intercept("GET", "/api/v2/maintenance/status", { body }).as(alias);
};

const interceptLogs = (body: unknown, alias: string) => {
