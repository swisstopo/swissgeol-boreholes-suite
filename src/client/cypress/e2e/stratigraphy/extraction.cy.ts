...
describe("Tests for stratigraphy extraction", () => {
  ...
  it("extracts two stratigraphies from a real profile and saves both", () => {
    ...
    // Save — one bulk POST to /api/v2/lithology carries both selected stratigraphies.
    cy.dataCy("add-stratigraphy-button").click();

    cy.wait("@lithology_POST", { timeout: 60000 }).then(interception => {
      const items = interception.request.body as Array<{ stratigraphy?: { name?: string } }>;
      const names = items.map(i => i.stratigraphy?.name).filter(Boolean);
      cy.log("Observed lithology POST stratigraphy names: " + JSON.stringify(names));
  ...
  it("navigates to the newly extracted stratigraphy after save when a primary already exists", () => {
    ...
    cy.get("@borehole_id").then(boreholeId => {
      ...
      cy.dataCy("add-stratigraphy-button").click();
      cy.wait("@lithology_POST", { timeout: 60000 }).then(interception => {
        const body = interception.response!.body as Array<{ stratigraphy: { id: number } }>;
        cy.wrap(body[0].stratigraphy.id).as("extractedId");
      });
