...
import { loadProjects } from "../../lib/index";
...
class ProjectDropdown extends React.Component {
  ...
  componentDidMount() {
    ...
    if (projects.data.length === 0 && projects.isFetching === false) {
  ...
  handleChange(event, data) {
    const { onSelected, projects, multiple } = this.props;
    if (multiple === true) {
      const selection = [];
  ...
  render() {
    const { projects, search, multiple } = this.props,
      { selected } = this.state;
